	...
	public class ItemsRepository
    {
		...
		public async Task<ulong> CreateItem(InventoryCard card, Player player)
		{
			...
			await _sqlObjectFactory.GetConnection().UsingAsync(connection =>
				...
				"VALUES (@PlayerId, @ItemId, @Period, @PeriodType, @Type, @Active, @Opened, @Giftable, @Boosted, @BoostLevel, @Time); SELECT LAST_INSERT_ID()", new { PlayerId = player.PlayerId, ItemId = card.ItemId, Period = card.Period, PeriodType = card.PeriodeType, Type = card.Type, Active = card.IsActive ? 1 : 0, Opened = card.IsOpened, Giftable = card.IsGiftable, Boosted = (card.BoostLevel > 0), BoostLevel = card.BoostLevel, Time=card.TimeCreated}));
			return items.Result;
		}
